...
final class AsyncCompleter<T: Sendable>: @unchecked Sendable {
    //
    struct WaitEntry {
        ...
        func cancel() {
            continuation.resume(throwing: UserKitError.cancelled)
            timeoutBlock.cancel()
        }

        func timeout() {
            continuation.resume(throwing: UserKitError.timedOut)
            timeoutBlock.cancel()
        }
    ...
    private let _timerQueue = DispatchQueue(label: "UserKit.AsyncCompleter", qos: .background)
